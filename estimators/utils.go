...
func ParseLearningPoint(s string) LearningPoint {
    sl := strings.Split(s,";")
    ...
    return LearningPoint{Y: fl[0], X: fl[1:len(fl)]}
}
...
func buildMeshCoordinate(inf, sup, step float64) []float64{
    nb_points := int(math.Ceil(sup - inf / float64(step) ))
    coordinates := make([]float64, 0, nb_points + 1)
...
func MeshEvalPoints(inf, sup, step float64, d int) ([]Point, error) {
    ...
    //
    nb_points := int(math.Ceil(sup - inf / step ))
    points := make([]Point, 0, (nb_points+1)*d )
